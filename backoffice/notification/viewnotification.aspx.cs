...
public partial class backoffice_notification_viewnotification : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            parameters.Clear();
            clsm.Fillcombo_Parameter("select distinct c.campus_name,c.campusid,c.displayorder from campus c  where c.status=1 order by c.displayorder", parameters, ddlcampus);
            bindcollege();
    ...
    {
        parameters.Clear();
        parameters.Add("@campusid", ddlcampus.SelectedValue);
        string str_college = "SELECT distinct  c.COLLAGENAME,c.COLLAGEID FROM collage_master c  WHERE c.STATUS=1  and c.campusid=@campusid";
        clsm.Fillcombo_Parameter(str_college, parameters, ddlcollege);
    ...
    public void gridshow()
    {
        string strsql = "";
        parameters.Clear();
        strsql = "select distinct a.*,b.ntype from Events a left join newstype b on a.ntypeid=b.ntypeid left join map_happening_campus mc on a.eventsid=mc.eventsid left join  map_institute_happenings  mf  on  mc.eventsid=mf.eventsid  where 1=1  and b.ntypeid=5    ";
        ...
        {
            parameters.Add("@campusid", Convert.ToInt32((ddlcampus.SelectedValue)));
            strsql += " and mc.campusid =@campusid";
        }
        if (ddlcollege.SelectedIndex != 0)
        {
            parameters.Add("@collageid", Convert.ToInt32((ddlcollege.SelectedValue)));
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            //if (lit_Eventsid.Text == "5" || lit_Eventsid.Text == "9")
            //{
            //    pnl_gallery.Visible = true;
            //}
    ...
    protected void GridView1_RowCommand1(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            Response.Redirect("notification.aspx?nid=" + Convert.ToInt32(e.CommandArgument));
        }
